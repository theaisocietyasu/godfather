...
from config.database import ssh_keys_collection
...
class SSHService:
    """Handle SSH key generation and management"""
    
    @staticmethod
    def get_or_create_org_key() -> Optional[Dict]:
        """Get or create organization-wide SSH key pair"""
        try:
    ...
    def save_key_to_temp_file(private_key: str) -> Optional[str]:
        """Save SSH private key to temporary file with correct permissions"""
        try:
